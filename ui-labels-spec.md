...
| 内部名 / 機能 | 表示ラベル | 備考 |
|---|---|---|
| reached | 到達 | ヘッダ主統計。`stage ≠ 'new'`（学習開始）の語数 `到達/総数` |
| mastered | マスター | `stage === 'mastered'` の語数（耐久マイルストン） |
| stable | 安定 | マスターかつ `pRecall ≥ targetRetention`（復習ライン下に沈黙）＝マスターの副統計 |
| reviewWait | 復習待ち | マスターかつ `pRecall < targetRetention`（半減して浮上）＝マスターの副統計。泡色 #CFEFFB |
| masteredCount | 定着語数 | （旧・sim/内部用）`stage === 'mastered'` の語数 |
| learnedCount | 学習済み語数 | stage ≠ 'new' の語数 |
| activeWaves | 現在のWave | アクティブウェーブの範囲 |
| excluded | 除外 | 学習対象外の語 |

ヘッダは「哲学として『定着』＝完了ではない」ため **到達 → マスター →（安定 / 復習待ち）** の階層で表示し、熟達で飽和する平均 h は撤去した。マスター済みの色分け（復習待ち＝泡リング／安定＝休眠ディム）・クリア解除（⚠）・引き潮の正直予測は spec.md §5.2 を参照。**※ Wave Heatmap 俯瞰バー（`ui-heatmap.js`）は young/泡ゲートは共有するが、復習待ち（due）の泡は未反映（時間依存のため currentTime/config の配線が要る）＝今後の整合タスク。**
