...
| #1 | h 更新が実経過時間（deltaT）を無視。`alpha_eff = 1 + (alpha−1)×min(1, deltaT/h)` 等で減衰させる案。挙動が変わるため sim 検証必須・最も価値が高い | 🟡 **コード実装済み・既定 false**（2026-06-11。`deltaTGain` フラグで gate）。素の式は target-retention と噛み合わず定着 ~20倍遅延と判明 → **次セッションで ratio 正規化等を校正し個体差ありの sim で検証**してから既定 true に。下記修正ログ参照 |
...
**再開のしかた**: #2〜#6 は完了。#1 はコード実装済みだが校正前（既定 false）。これらは `bayesian-srs-proposal.md` で統合的に再構成された（#5 を「naive 削除」ではなく「偽の実装を消し、導出関数 `uncertaintyWidth` + サンプリングで本来の構想を回復」する方向）。**ユーザー合意の進行順**:

1. ✅ **共有土台（2026-06-11 完了）**: 仮想学習者に語ごと真 h の ±30% 個体差を導入（提案書 §8 の検証注意点。#1・提案の両方で「推定誤差がないと真価を測れない」ため必須）＋ 死んだ μ/σ/uncertain プールの削除
2. ✅ **提案 Phase 1（2026-06-11 完了）**: `uncertaintyWidth(word, currentTime)` 導出関数 + due 判定の `effectiveH` トンプソンサンプリング → シナリオ E（`scripts/verify_due_sampling.js`）で位相同期分散を sim 検証（ピーク需要 75→67・定着 +7%・回帰なし）。新パラメータ `uncertaintyBase`(0.5)・`uncertaintyFloor`(0.05)・`staleGrowth`(0.05)・`dueSampling`(true) を追加
3. 🟡 **#1: コード実装済み・既定 false（2026-06-11）→ 次セッションで校正＋検証**: `deltaTGain` フラグで gate 済み。**素の式 `min(1, deltaT/h)` は target-retention スケジューリング（復習時 deltaT≈h×0.234）と噛み合わず定着 ~20倍遅延**と判明（下記 #1 修正ログ参照）。次セッションは **ratio 正規化 `deltaT/(h×retentionFactor)`**（予定どおり復習＝full gain）または `(1−p)` スケーリングを校正し、**個体差ありの sim で before/after 検証**してから既定 true に倒す
4. 提案 Phase 2/3（任意・urgent 多次元トリアージ・UI 信頼性表示・確認モード）

#1 と提案は「捨てている deltaT を使う」共通の根を持ち、手を入れる場所は違う（#1=h 成長量・提案=スケジューリング/不確実性）ので**補完関係**。サンプリングが due 時刻を散らす → deltaT 多様性 → #1 の旨味が出る、という正の相乗効果がある。判定変更時は memory `feedback_srs_policy_single_source.md`（SRS ポリシーは core 一元化・app/sim は同じ経路）の原則に従い、各ステップで sim before/after を回すこと。
...
### review.md #1: deltaT 連動の h ゲインを実装（gated・既定 false。校正と検証は次セッション）

`review.md` の指摘 #1（h 更新が実経過時間 deltaT を無視）に対応する**コード経路を実装**。正解時のゲインを前回復習からの経過時間で減衰させる:
...
- `core/srs-engine.js` `_updateHalfLife(word, cardType, isCorrect, result, currentTime)`: 第5引数 `currentTime` を追加（`word.lastReviewed` 更新前に呼ばれるので deltaT = currentTime − 前回復習時刻）。`cfg.deltaTGain` で gate
- cardWeight は alpha の全体倍率ではなく**ボーナス項**に掛ける（`1 + (alpha−1)×cardWeight×ratio`）。これで正解時に h が縮まない不変条件を保つ（旧 `h×alpha×cardWeight` は recognition cardWeight=0.8 と素の alpha_eff≈1 で h<元 になり得た）
- `core/config.js`: `deltaTGain` フラグ追加

**重大な発見（既定 false にした理由）**: 既定 ON で sim を回すと**定着が ~20倍遅延**した（D90 定着 19-28 vs 旧 484-494、D180 38-50 vs 845）。バグではなく**素の式が target-retention スケジューリングと構造的に噛み合わない**ため:
- スケジューラは `targetRetention=0.85` で復習する → 復習時の `deltaT ≈ h × log₂(1/0.85) = h × 0.234`
- つまり**予定どおり復習すると常に `deltaT/h ≈ 0.234`**（1 には決して届かない）→ `gain ≈ 1 + (alpha−1)×cardWeight×0.234 ≈ 1.2` で頭打ち
- h が伸びると復習間隔（h×0.234）も比例して伸びるため ratio は 0.234 に固定され続け、永久に full gain に届かない。h は ×2 でなく ×1.2 ずつしか伸びず mastered(h≥14) に到達しない
- 素の `min(1, deltaT/h)` は「忘却の縁（deltaT≈h, p≈0.5）で復習する」前提だが、実スケジューラは p≈0.85 で復習するため乖離する

**対応**: アプリのプロトタイプを壊さないよう **`deltaTGain` の既定を false**（旧挙動を維持）にし、新コード経路は次セッションで校正・検証して有効化する。校正の有力案は **ratio の正規化** `ratio = min(1, deltaT / (h × retentionFactor))`（予定どおり復習＝ratio 1＝full gain、予定より早い復習ほど減衰）。または `(1−p)` スケーリング。default false の標準パスは sim 回帰なし（D90 定着 ~499）。

**次セッション**: 個体差ありの sim で deltaTGain の校正（ratio 正規化等）と before/after 検証 → 妥当なら既定 true に。`scripts/verify_due_sampling.js` と同様の検証スクリプトを用意し、キャリブレーション（実 p と予測 p のずれ）改善を測る。
...
**検証（`scripts/verify_due_sampling.js` 新規・シナリオ E）**: 朝集中学習者（6分間隔×5/朝）×30日・5回平均で OFF/ON 比較。点推定 h 基準の復習需要を中立指標として測定。
- **ピーク需要 max 75 → 67**（同時 due の最大スパイクが縮小＝位相同期が散った）、backlog mean 23.9 → 21.0
- **定着 218 → 233（+7%）・avgH 28.7 → 31.2**（早めにサンプリングされた語が overflow 中に先に処理され throughput 微増）。復習なしは両者0（この高負荷学習者は谷が出ない）
- CV は mean 低下に伴い横ばい（0.99→1.02・CV はこの局面では適切な指標でない）

`sim/scenarios.js` にシナリオ E（`dueSampling` off/on・頻回学習者）を追加。**標準学習者（3回/日）での macro 影響は ON/OFF ともほぼ同等**（D90 ~475-497・D180 ~840-864・ノイズ範囲内＝スケジューリングのゆらぎで throughput は不変・回帰なし）。app も同じ core 経路を通るため「若い記憶はゆらぎ、成熟した記憶は安定する」挙動が反映される（`_computeTide` の予測は点推定 h のまま＝期待値ベースで安定）。

**次**: #1（deltaT 連動の h 更新。`alpha_eff = 1 + (alpha−1)×min(1, deltaT/h)` 等）→ 個体差ありの sim でキャリブレーション改善を検証。続いて提案 Phase 2/3（urgent 多次元トリアージ・UI 信頼性表示・確認モード）。
...
## シミュレーション実績（waveSize=100・供給ベース wave 解放・定着=`stage === 'mastered'`・仮想学習者 ±30% 個体差込み）

| Day | 定着語数 | 学習済み | avgH | Wave |
|-----|--------|--------|------|------|
| 30  | ~173-188 | ~228-240 | ~37日 | [2,3] |
| 60  | ~335-353 | ~377-400 | ~70日 | [4] |
| 90  | ~473-497 | ~515-540 | ~100日 | [5,6] |
| 180 | ~818-886 | ~861-914 | ~182日 | [9,10] |
| 223 | ~1000 | ~1045 | ~200日 | — |

正解率 75〜85%、Wave は just-in-time で1波ずつ解放（19波体制）、**1000語定着が Day ~223-226 で到達**。2026-06-11 のステップ1で仮想学習者に語ごと真 h の ±30% 個体差を導入（推定誤差を持たせる共有土台）。これにより推定が外れる語が増え後半ほど保守化し、個体差なしの旧ベースライン（Day 90 ~495・Day 180 ~870・1000語 Day ~214）から D180 ~830・1000語 Day ~224 に補正された。app の挙動は不変（core の μ/σ/uncertain 削除は判定経路を変えない）。
