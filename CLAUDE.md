...
1. **`waveSize` 縮小**（50→25語）と **`maxNewPerSession` 増加**（5→7語）で meaningful 100枚/日を達成できるか検証
2. **Wave unlock 条件を review 回数ベースに変更**し、h 成長速度から切り離す
3. **「学習者ライフスタイルへのアダプティブ動的 Word Wave」**：1日のセッション間隔を検出し、パラメータを自動調整するコンセプトが浮上
