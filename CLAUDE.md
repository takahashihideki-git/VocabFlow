...
| #5 | ベイズ層（μ/σ）が形骸化（μ は死にフィールド・σ は不正解でも減少・uncertain プールほぼデッドコード）。削除して簡素化 or 活用 | ⬜ **次はここ推奨**（簡素化の好機。削除方向） |
| #1 | h 更新が実経過時間（deltaT）を無視。`alpha_eff = 1 + (alpha−1)×min(1, deltaT/h)` 等で減衰させる案。挙動が変わるため sim 検証必須・最も価値が高い | ⬜ 未着手（大きめのテーマ） |
...
**再開のしかた**: 残りは #5（ベイズ層の形骸化）と #1（h 更新の deltaT 無視）。この2つは `bayesian-srs-proposal.md` で統合的に再構成されている（#5 を「naive 削除」ではなく「偽の実装を消し、導出関数 `uncertaintyWidth` + サンプリングで本来の構想を回復」する方向）。**ユーザー合意の進行順**:

1. **共有土台**: 仮想学習者に語ごと真 h の ±30% 個体差を導入（提案書 §8 の検証注意点。#1・提案の両方で「推定誤差がないと真価を測れない」ため必須）＋ 死んだ μ/σ/uncertain プールの削除
2. **提案 Phase 1**: `uncertaintyWidth(word, currentTime)` 導出関数 + due 判定の `effectiveH` トンプソンサンプリング → シナリオ E で位相同期分散を sim 検証
3. **#1**: deltaT 連動の h 更新（`alpha_eff = 1 + (alpha−1)×min(1, deltaT/h)` 等）→ 個体差ありの sim でキャリブレーション改善を検証
4. 提案 Phase 2/3（urgent 多次元トリアージ・UI 信頼性表示・確認モード）
...
## シミュレーション実績（waveSize=100・供給ベース wave 解放・定着=`stage === 'mastered'`）

| Day | 定着語数 | 学習済み | avgH | Wave |
|-----|--------|--------|------|------|
| 30  | ~185-205 | ~230-240 | ~37日 | [2,3] |
| 60  | ~345-355 | ~370-400 | ~70日 | [4,5] |
| 90  | ~490-500 | ~515-565 | ~105日 | [6] |
| 180 | ~860-885 | ~878-910 | ~185日 | [10,11] |
| 214 | ~1000 | ~1045 | ~200日 | — |

正解率 75〜85%、Wave は just-in-time で1波ずつ解放（19波体制）、**1000語定着が Day ~214 で到達**（2026-06-11 の定着定義 stage 統一で h≥14 比やや保守化。降格して h は高いが stage=dictation の語を除外するため後半ほど差が広がり、旧 h≥14 定義の Day 90 ~506 → ~495、Day 180 ~965 → ~870 に補正）。
