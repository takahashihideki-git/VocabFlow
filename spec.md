...
6. Filler スロットは Passive（読み物）と既知語の Intro（単語・意味・例文の再提示）を混合する。Passive だけが連続しないようにする。既知語の Intro は h 更新なし（通常の Intro と同じ扱い）。将来の動画化で、このスロットが単語イメージ動画の再生に自然に移行する。
