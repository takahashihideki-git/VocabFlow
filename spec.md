...
> **新語供給を「予約」する変種は不採用（対オラクル検証の結論・2026-06-25）**
> 「1日N新語を復習と独立に予約する（reserveNewSlots）」「観測成功率で予約枠を動的に絞る（adaptiveNew）」「成功率が低くても最低 floor 語は導入する（adaptiveNewFloor）」の3変種を、真の忘却カーブを族（指数則／べき則／ACT-R）として総当たりし、絶対アウトカム（mastered＝ステージ成熟・avgH）で検証した。**いずれも greedy（上記の review-first 有機 throttle）に劣る**ため不採用とした（gated・既定オフで残置）。
>
> - 予約型は容量限界下で**早期過剰導入を招き、既存語が成熟しないデススパイラル**に陥る（指数則 novice で mastered 264→2・avgH 88→1）。新語供給の不足ではなく**成熟スロットの枯渇**が原因なので、フロアでは塞げない（導入は増えるが avgH をさらに薄める）。
> - greedy の「new = 復習の余りスロット」は、復習圧を**無限の backlog ではなく新語導入の鈍化**に変換する逃し弁として働く。これが真実不確実性に対する頑健性の正体。
> - ただし**過剰復習は間隔反復の本質的な危険側**（復習負債の雪だるま＝review wall）。指数則 HLR がべき則真実の下で h を過小評価すると復習を早めにスケジュールし、復習予算を食う。この税は対オラクル効率ギャップとして現れ、**復習需要が容量に張り付く初学者層で最も効く**。真の忘却カーブの形は sim では決着できず、実機の復習ログでのみ測れる（唯一の外部審判）。詳細は `memory-core-investigation.md`。
...
### 5.2 カラーマッピング（水深ランプ / bathymetric）

定着＝深く沈んで凪いだ深海、というマッピング。h が育つほど語は水面から深部へ沈む。海メタファー（Ripple/Wave/Tide）を一覧性チャネルにも徹底するため、旧・火/紅葉ランプ（赤→緑）を海の鉛直プロファイルに置き換えた（2026-06-18）。抜けかけた語（h<1）は赤の警告ではなく「陽の差す浅瀬」で、緊急シグナルは水面の波立ち表現に委ねる（Word Wave は学習フィードではなく俯瞰の地図のため、緊急度より記憶強度を語る）。

| h の範囲 | 色 | 状態 |
|---|---|---|
| 未学習 | グレー (#333348) | まだ水に入っていない陸 |
| h < 1日 | 浅瀬ターコイズ (#2FD9C5) | 覚えたて、最も揺らぐ水面 |
| 1日 ≤ h < 3日 | 浅青 (#29A9C2) | Recognition〜Recall段階 |
| 3日 ≤ h < 7日 | 中層青 (#2486BC) | Dictation段階 |
| 7日 ≤ h < 14日 | 中層 (#2566AC) | 定着進行中 |
| 14日 ≤ h < 30日 | 深い青 (#244F9E) | ほぼ定着 |
| h ≥ 30日 | 深海インディゴ (#1B2E66) | 完全定着 |

mastered（定着）の可視化は**波の消失**で表す: 未定着の語はボックス下端に1本のゆるい曲線（奇数=左傾き／偶数=右傾きの反転で俯瞰では波の連なりに見える）を持ち、まだ水面で揺れている。mastered になると曲線が消え、凪いだ深海に沈んだクリーンな箱になる（旧・金ドット表現を廃止）。クリア Wave の金バッジ（#f7d774）は深青の上の暖色アクセント（海底の宝）として据え置き。

#### マスター済みの 2 状態＝安定 / 復習待ち（2026-07-01）

色を h（安定性）だけで塗ると、熟達者では h ティアが飽和して大半が最深 t5 に張り付き、**色チャネルが死ぬ**（＝「すっかりやりきった」見た目）。しかも h は忘れにくさ（安定性）であって**いま引き出せるか（想起可能性）**ではないため、記憶が半減して復習ラインを割った語（`pRecall < targetRetention`）が、凪いだ深海の語と一覧上まったく同じに見える。これは「記憶は生き物」という本アプリの哲学（Wave クリア overlay）にグリッドが反する状態だった。

そこでマスター済みを 2 状態に分ける（`ui-wordwave.js _applyColor`）:
- **復習待ち（due）** = `stage==='mastered'` かつ `pRecall < targetRetention`。記憶が半減して**浮上**した語。深さ色は残したまま、**波頭の泡クレスト色 #CFEFFB の輪郭リング**（`box-shadow`）で囲う。box-shadow なので底の波（`::after`）と描画レイヤーが別＝重なっても潰れない。泡色 #CFEFFB は「出会ったばかり」#9FD8E8 と隣接させても衝突しない明度差を持つ。
- **安定（dormant）** = マスター済みかつ not due。まだ復習ラインの下に沈んで眠る語。**文字色を落として（#5a6a99）休眠**させ、浮上した due だけが際立つようにする。

未マスター（学習中）の語はこの分岐に入らず、従来どおり底の波（frontier ripple・`ww-word--active`）で「まだ水面」を示す。したがってグリッドは **眠る深海（安定・沈黙）／泡リングで浮上（復習待ち・覚醒）／先端で生きる波（未マスター ripple）** の三層で読める。h の点推定値はポップオーバーで常に閲覧可能（色という一覧性チャネルは「いまの想起可能性」だけを語る）。

ヘッダ統計もこの意味論に合わせて **到達（学習開始）→ マスター（mastered 到達）→ 安定 / 復習待ち** の階層に変更し、飽和して無意味な平均 h は撤去した（`_updateStats`）。

#### クリア解除の可視化（2026-07-01）

Wave のクリアは「非除外語が全 mastered」で判定するが、降格で 1 語でも mastered を割ると解除される。旧 h ベースの色では、降格した語（例: `patient` h=40 で dictation 止まり）が深部マスター風に見え、その Wave が金の「クリア済み」ラベルのまま**解除が隠れて**いた。新設計では `everClearedWaves`（過去履歴）に含まれるが現在は未達の Wave のラベルを、金を剥がし **⚠ 破線**（`ww-wave-label.revoked`）で表示し、「一度沈めたが浮いてきた」ことを正直に出す。

#### 引き潮の正直な予測（Tide・2026-07-01）

新語ハードル（`feed-generator` の貪欲割当で `新語枠 = sessionSize − 復習需要`）を Tide の潮メタファーに統合する。旧実装は「次の満ち潮は約N日後」を**生涯平均セッションペース**で外挿し、かつ復習バックログを**一度きりの山**とみなしていたため、実際には復習が湧き水のように補充される定常状態（熟達者）で「1〜2日後」を出し続ける不正直な表示だった。新実装（`_computeTide` / `_updateStats`）は:
- **主軸＝作業量**: 「満ち潮まで あと約N語（約M セッション）」。N = `reviewDemand −(sessionSize−3)`。ユーザーが動かせる単位で正直。
- **従＝日数**: 直近実測の消化速度（`throughput` = 直近7日で触れた語数/日）から湧き水（`influx` = 翌日 due になる語数/日）を差し引いた `netDrain` で日数化。`netDrain ≤ 0` なら「現ペースでは復習待ちが減りません — 1日の学習量を増やすと満ちます」と明示（生涯平均でごまかさない）。
- 引き潮の**水位を復習需要で連続化**（旧: 一律 52%）。需要が重いほど水位が低く、満ち潮ラインへの距離＝ハードルが見える。

#### 信頼度ゲート（Word Wave 単語一覧・Wave Heatmap 共通）

色は「色 = h の値」ではなく **「色 = 確認された記憶強度」** の意味論を採る。導入直後（`reviewCount` 1〜2）の h は ripple 播種ノイズ（rc=1 の一撃で散らした h0）を多く含み、語間の色差は「確認された記憶の差」ではない。これを水深ランプに反映すると**ノイズを実力差として見せる嘘**になるため、信頼度が立つまでは一律の「出会ったばかり」色に保つ。Word Wave 単語一覧（`ui-wordwave.js`）と Wave Heatmap 俯瞰バー（`ui-heatmap.js`）の両方が同じゲートを通る（閾値定数 `CONFIDENCE_MIN_REVIEWS` を `core/labels.js` に一元化）。

| 条件 | 色 | 状態 |
|---|---|---|
| `stage === 'new'` | グレー (#2A2A3D) | 未導入（まだ出会っていない） |
| `reviewCount < 3` かつ導入済み | 泡（白アクア） (#9FD8E8) | 出会ったばかり・まだ信頼度が立たない |
| `reviewCount ≥ 3` | 上表の h ティア（浅瀬〜深海） | 測定済みの記憶強度 |

三段階の色文法：**グレー（未学習）→ 泡（出会ったばかり、まだ分からない）→ 水深グラデ（測定済み）**。泡（白アクア・出会ったばかり）が水深ランプの最浅・最明としてそのまま浅瀬ターコイズに連続するため、三段階が一本の水の流れ（水面の泡 → 浅瀬 → 深海）として読める。「まだ分からない」に専用の一色を与えることは、ベイジアン不確実性（§1.3）の可視化そのもの。閾値に `uncertaintyWidth`（§1.3）ではなく `reviewCount` を使うのは、前者の `staleFactor` が長期放置された熟知語を泡に再降格させ「確認された記憶強度」の意味と矛盾するため。h の数値自体はポップオーバー／ツールチップで常に閲覧可能（色という一覧性の高いチャネルでのみ、確認された差だけを語る）。
